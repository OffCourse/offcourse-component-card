...
describe "Card Component", ->
  ...
  Given -> 
    ...
    @model = { bar: "bar", schema: ["baz"] }
    @context = "main"

  describe "normal schema", ->
    When  ->
      @subject  = renderElement @Component, { @model, @context }
    Then  ->
    ...
    And   ->
      ...
      props = { @model, schema: @model.schema }
  ...
  describe "nested schema", ->
    Given ->
      @spy.reset()
      @model.schema = { main: ["baz"] }
    When  ->
      @subject  = renderElement @Component, { @model, @context }
    Then  ->
      cardContentProps = @spy.getCall(0).args[0]
      props = { @model, schema: @model.schema[@context] }
      expect(cardContentProps).to.deep.equal(props)
  ...
  describe "only child is a header image", ->
    Given ->
      ...
      @children = "qux" 
    When  ->
      @subject  = renderElement @Component, { @model, @context, @children }
    And   ->
